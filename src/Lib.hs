...
samplePuzzle = Puzzle
  ...
  , [ Val Four  , Empty     , Empty     , Val Eight, Empty    , Val Three, Empty    , Empty    , Val One  ]
...
isValid (Puzzle rows) =
  ...
  in
    all listValid cols && all listValid rows && all listValid boxes
...
makeBoxes rows =
  let
    ...
    allCoords :: [(Int, Int)]
    allCoords = toTuples $ concat $ mapM (const [0,3,6]) [1..2]
  in
    map (boxAt rows) allCoords
...
listValid xs =
  let
    ...
    hasOneOrFewer list val =
      length (filter (val ==) list) <= 1

  in
    all (hasOneOrFewer xs) filledVals
